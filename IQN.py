...
class IQN(nn.Module):
    def __init__(self, state_size, action_size, layer_size, n_step, seed, layer_type="ff"):
        ...
        # weight_init([self.head_1, self.ff_1])  # before #
    ...
    def forward(self, input, num_tau=8):
        ...
        x = (x.unsqueeze(1) * cos_x).view(batch_size * num_tau, self.layer_size)  # x * cos_x
...
def run(frames=1000, eps_fixed=False, eps_frames=1e6, min_eps=0.01, collision=0, success=0, writer=None):
    ...
    for frame in range(1, frames + 1):
        ...
        state = next_state
        score += reward
        # linear annealing to the min epsilon value until eps_frames and from there slowly decease epsilon to 0 until
        # the end of training
        if eps_fixed == False:
            if frame < eps_frames:
        ...
        if done:
            ...
            if i_episode >= 20 or score < -500:
                writer.add_scalar("IQN Distributional RL reward", score, frame)
            ...
            if i_episode % 100 == 0:
                print(
                    '\rEpisode {}\tFrame {}\tAverage Score: {:.2f}\tCollision Rate: {:.2f}\tSuceess Rate: {:.2f}'.format(
                        i_episode, frame, np.mean(scores_window), collision / 100, success / 100))
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    final_average100 = run(frames=100000, eps_fixed=eps_fixed, eps_frames=5000, min_eps=0.025, collision=0, success=0,
                           writer=writer)
    t1 = time.time()

    print("Training time: {}min".format(round((t1 - t0) / 60, 2)))
    torch.save(agent.qnetwork_local.state_dict(), "IQN100" + ".pth")
